...
router.get('/get/html', function(req, res) {
    ...
    var xml = fs.readFileSync('GuitarCenter.xml', 'utf8'); //We are reading in the XML file
    var xsl = fs.readFileSync('GuitarCenter.xsl', 'utf8'); //We are reading in the XSL file
...
router.post('/post/json', function (req, res) {
    ...
    function appendJSON(obj) {
        ...
        xmlFileToJs('GuitarCenter.xml', function (err, result) {
            if (err) throw (err);
            
            result.cafemenu.section[obj.sec_n].entree.push({'item': obj.item, 'brand':obj.brand, 'price': obj.price});

            console.log(JSON.stringify(result, null, "  "));

            jsToXmlFile('GuitarCenter.xml', result, function(err){
